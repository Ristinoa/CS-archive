...
def secondaryTroop(title, path):
    ...
    if result is None:
        ...
        # 3/21/2023 I gues it also doesn't really matter since the only other troop with a gimmick like this
        # was the mega lava hound, which was a ltd. time event troop
...
def writeStatsToFile():
    ...
    file3 = open('spellAttributes.txt', 'w')
    for card in cards:

        troop = getCardData(card['title'],card['link'])

        if troop == 'Spell':
            # file3.write(str(troop) + '\n')
            pass
        else:
