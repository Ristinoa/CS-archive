...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state: State, action: str) -> List[PossibleResult]:
        ...
        # print("Start of new function call!\n")
        # print("Starter state: ")
        # print(state)
        ...
        if action == 'Take':
            # check to see if next card is known
            if state.nextCard == None:
                ...
                for card in cards:
                    ...
                    if card > 0:
                        temp = None
                        reward = 0
                        newHandTotal = state.handTotal + self.cardValues[tracker]
            
                        if newHandTotal > self.threshold:
                            reward = 0
                        
                        
                        elif newHandTotal <= self.threshold and sum(cards)-1 > 0:
                       
                            left = cards[0:tracker]
                            right = cards[tracker+1:len(cards)]
                            
                            temp = left + tuple([card-1]) + right
                        ...
                        newState = State(handTotal=newHandTotal, nextCard=None, deckCounts=temp)
                        possibleResults.append(PossibleResult(successor=newState,
                                                            probability=(float(card/dtotal)),
                                                            reward=reward))
                    tracker += 1 
            # if next card IS known:       
            else: 
                temp = None
                reward = 0
                ...
                if newHandTotal <= self.threshold and sum(cards)-1 > 0:               
                            left = cards[0:state.nextCard]
                            right = ()
                            if state.nextCard + 1 < len(cards):
                                right = cards[state.nextCard+1:len(cards)]
                            
                            temp = left + tuple([cards[state.nextCard]-1]) + right
...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state: State, action: Any, reward: int, newState: State) -> None:
        ...
        # stand in for our feature function value:
        V_optimal = 0.0

        if newState is not None:
           # V(s', a')
           V_optimal = max([self.getQ(newState,newAction) for newAction in self.actions(newState)])
    
        # carry on building update rule:
        Q_optimal = self.getQ(state,action)
        adjustment = -self.getStepSize()       
        difference = (Q_optimal - (reward + self.discount * V_optimal))
        # update weights:
        for item in self.featureExtractor(state,action): 
            key,value = item
            # update rule: w_i = w_i + adjustment * difference * f_i (s,a)
            self.weights[key] = self.weights[key] + adjustment * difference * value
...
def blackjackFeatureExtractor(state: State, action: str) -> List[Feature]:
    # required variables 
    # sadly, cannot remove nextCard or whole thing explodes
    total, nextCard, counts = state

    # structure of feature function is binary, since there's only one optimal choice per state
    featureValue = 1
    featureKey = (action,total)

    # same strategy as feature extractor given to us above by sample
    ffv = []
    ffv.append((featureKey,featureValue))
    ...
    if counts != None:
        ...
        for index in range(len(countsList)):     
            featureKey = (action,index, countsList[index])
            ffv.append((featureKey, featureValue))
            if countsList[index] > 0:
                countsList[index] = 1
        # update feature key and add to feature function 
        featureKey=(action, tuple(countsList))
        ffv.append((featureKey, featureValue))
    
    # regardless of outcome, return feature function
    return ffv
