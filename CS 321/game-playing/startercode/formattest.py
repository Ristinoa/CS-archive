...
def succAndProbReward(self, state: State, action: str) -> List[PossibleResult]:
    ...
    if action == 'Take':
        ...
        if state.nextCard is None:
            for tracker, card in enumerate(cards):
                if card > 0:
                    newHandTotal = state.handTotal + self.cardValues[tracker]
                    reward = 0 if newHandTotal > self.threshold else newHandTotal if newHandTotal <= self.threshold else newHandTotal if dtotal == 0 else 0
                    temp = None if newHandTotal > self.threshold else cards[:tracker] + tuple([card - 1]) + cards[tracker + 1:]
                    newState = State(handTotal=newHandTotal, nextCard=None, deckCounts=temp)
                    possibleResults.append(PossibleResult(successor=newState, probability=(float(card / dtotal)), reward=reward))
        else:
            newHandTotal = state.handTotal + self.cardValues[state.nextCard]
            temp = None if newHandTotal > self.threshold else cards[:state.nextCard] + tuple([cards[state.nextCard] - 1]) + cards[state.nextCard + 1:]
            reward = 0 if newHandTotal > self.threshold else newHandTotal if newHandTotal <= self.threshold else 0
            newState = State(handTotal=newHandTotal, nextCard=None, deckCounts=temp)
        ...
        for tracker, card in enumerate(cards):
            if card > 0:
                newState = State(handTotal=state.handTotal, nextCard=tracker, deckCounts=state.deckCounts)
                possibleResults.append(PossibleResult(successor=newState, probability=float(card / dtotal), reward=0 - self.peekCost))
