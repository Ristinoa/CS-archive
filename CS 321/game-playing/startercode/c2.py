...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state: State, action: str) -> List[PossibleResult]:
        ...
        if action == 'Take':
            ...
            numCards = sum(cards)
            for card in range(len(state.deckCounts)):
                #If there is a card to draw for an action
                if state.deckCounts[card] > 0:
                    reward = 0
                    newDeckCounts = list(state.deckCounts)
                    newHandTotal = state.handTotal + self.cardValues[card]
                    # print("new hand total")
                    # print(newHandTotal)
                    newDeckCounts[card] = newDeckCounts[card] - 1
                    
                    #A player has "gone bust"
                    if newHandTotal > self.threshold:
                        reward = 0
                        newDeckCounts = None

                    #A player has run out of cards to draw
                    elif sum(newDeckCounts) == 0:
                        reward = newHandTotal
                        newDeckCounts = None
                    
                    else:
                        newDeckCounts = tuple(newDeckCounts)   

                        
                    newState = State(handTotal=newHandTotal, nextCard=None, deckCounts=newDeckCounts)
                    possibleResults.append(PossibleResult(successor=newState,
                                                        probability=(float(state.deckCounts[card]/numCards)),
                                                        reward=reward))
            ...
            for card in range(len(state.deckCounts)):
                if state.deckCounts[card] > 0:
                    ...
                    newState = State(handTotal=0, nextCard=card, deckCounts=state.deckCounts)
                    possibleResults.append(PossibleResult(successor=newState,
                                                        probability=(state.deckCounts[card]/numCards),
